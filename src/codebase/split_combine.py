...
class SplitComb():
    ...
    def split(self, data, side_len = None, max_stride = None, margin = None):
        ...
        splits = []
        _, z, h, w = data.shape           # data只有4个维度，

        nz = int(np.ceil(float(z) / side_len))   # 可以分为几块，向上取整
        ...
        # 填充
        pad = [ [0, 0],
                [margin, nz * side_len - z + margin],   # 先将各个维度补全为side_len的整数倍，再从两边补充margin
        ...
        for iz in range(nz):
            for ih in range(nh):
                for iw in range(nw):
                    ...
                    split = data[np.newaxis, :, sz:ez, sh:eh, sw:ew] # 添加了新的维度，每个小立方体的维度为5
                    splits.append(split)      # 将这些小立方体串联起来
