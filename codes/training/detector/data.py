...
class DataBowl3Detector(Dataset):
    """
    Dataset，由torch提供，必须实现其中的 __getitem__ 以及 __len__ 方法
    """
    def __init__(self, data_dir, split_path, config, phase = 'train',split_comber=None):
        """
        
        :param data_dir: <str> 预处理结果的存放路径
        :param split_path: <str> npy文件的路径 该ndarray存放所有CT的id
        ...
        if self.phase != 'test':
            ...
            for i, l in enumerate(labels):
                if len(l) > 0 :
                    # t就是对一个肺结节的标记，z y x dia
                    # 这个地方是应对  不同直径肺结节样本  不均衡的问题
                    for t in l:
                        if t[3]>sizelim:
                            # id,d,h,w,dia
            ...
            # 有些id信息的标记 shape:(N,5) 5分别表示 id z h w dia
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        :param imgs: 3D CT ndarray图像
        :param target: 一个肺结节标注信息的  D H W dia
        :param bboxes: 一个CT的所有肺结节标注信息
        :param isScale: bool
        :param isRand: bool
        :return: 
        crop : 来自 imgs
