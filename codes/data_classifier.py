...
class DataBowl3Classifier(Dataset):
    def __init__(self, split, config, phase = 'train'):
        """
        usage: dataset = DataBowl3Classifier(testsplit, config2, phase = 'test')
        
        :param split: [str] 样本id的list
        :param config: dict net_classifier.py中的配置项
        :param phase: str
        """
        ...
        for idx in idcs:
            # 一个CT对应一个 _pbb.npy 以及 _lbb.npy 文件
